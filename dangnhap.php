...
if(isset($_GET['login'])){
    echo"<script>alert('Đăng nhập để mua!');</script>";
}
if(isset($_POST['btn'])){
 $email = $_POST['email'];
 $password = $_POST['password'];

 $sql= "SELECT * from khachhang where (email='$email' or username='$email')  and  password='$password'";
 ...
 if( (mysqli_num_rows($result)>0)){
        ...
        $email= $row['email'];
        $phone= $row['phone'];
        $_SESSION['inforuser']=['ten' => $ten,'email' => $email,'id' => $id,'phone' => $phone];
